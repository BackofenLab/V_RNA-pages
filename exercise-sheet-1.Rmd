...
Conserting loop length of 1 means at least one unpaired nucleotide that must exist in a loop.
:::
...
if `$j \leq i$`
   return
else if $N_{i,j}$ = $N_{i,j-1}$ than
   trackback(i,j-1);
   return
else
   for all $k: i \leq k < j$, $S_{k}$ and $S_{j}$ complementary do
