---
title: "Exercise sheet 5: Sankoff algorithm"
---
...
$$
  ...
  \begin{cases}
    e(s_{1}) + e(s_{2}); s_{1}, s_{2}\text{ hairpin closing bp }(i_{1}, j_{1})(i_{2},j_{2})\\
    min_{p_{1},q_{1},p_{2},q_{2}} \{e(s_{1}) + e(s_{2}) + C(p_{1},q_{1},p_{2},q_{2})\}; \parbox[t]{3cm}{$s_{1}, s_{2}$ are 2-loops enclosed by $i_{1},j_{1},i_{2},j_{2}$ with pq accessible}\\
...
knitr::include_graphics("assets/figures/exercise-sheet-5/e1-1.jpeg")
```
...
```{r, include=knitr::is_html_output(), echo=FALSE,  fig.align='center', out.width='50%'}
knitr::include_graphics("assets/figures/exercise-sheet-5/e1-2.jpeg")
```
...
here: $O(n^4)*O(n^{4*3}) = O(n^{16})$

without upper bound on $k$: $O(n^{4(k-1)+4})$ or $O(n^{4*k})$
